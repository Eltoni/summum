...
class ParcelasContasPagarInline(admin.TabularInline):
    ...
    fields = ('id', 'num_parcelas', 'contas_pagar', 'formata_data', 'valor', 'encargos_calculados', 'valor_total', 'valor_pago', 'valor_a_pagar', 'link_pagamento')
    readonly_fields = ('id', 'num_parcelas', 'contas_pagar', 'formata_data', 'encargos_calculados', 'valor_total', 'valor', 'valor_pago', 'valor_a_pagar', 'link_pagamento')
    extra = 0
    can_delete = False
...
class ContasPagarAdmin(admin.ModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        self.fieldsets = (
            ...
            (None, {
                ...
                'fields': ('valor_total_juros', 'valor_total_multa', 'valor_total_encargos', 'valor_total_cobrado', 'valor_total_pago', 'valor_total_a_pagar')
            }),
        )
        ...
        if obj is None:
            self.fieldsets[1][1]['fields'] = tuple(x for x in self.fieldsets[1][1]['fields'] if (x!='valor_total_pago' and x!='valor_total_cobrado' and x!='valor_total_a_pagar' and x!='valor_total_encargos' and x!='valor_total_juros' and x!='valor_total_multa'))
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if obj:
            return ['status', 'id', 'compra_associada', 'valor_total', 'data', 'descricao', 'fornecedores', 'forma_pagamento', 'grupo_encargo', 'valor_total_pago', 'valor_total_juros', 'valor_total_multa', 'valor_total_encargos', 'valor_total_cobrado', 'valor_total_a_pagar',]
        else:

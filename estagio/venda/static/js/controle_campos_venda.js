...
$(document).ready(function(){
    ...
    if ( tem_data == '(Nenhum)' ) {                                 // e verifica se data não foi inserida
        $(".field-status, .field-data, .field-pedido, .field-status_pedido").each(function(){            // caso não tenha sido inserida, significa que a venda não foi registrada no banco
        ...
        (function($) {
            $(document).ready(function($) {
                    ...
                    $.ajax({       
                        type: "GET",
                        url: "/checa_pedido_venda_habilitado/"+id,
                        context: document.body,
                        dataType: "json",
                        success: function(retorno){
                            $.each(retorno, function(i, exibe_botao_pedido_venda){
                                var pedido_compra = exibe_botao_pedido_venda.fields['habilita_pedido_venda'];
